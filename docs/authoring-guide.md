# 글 작성 · 분류 가이드 (Authoring & Taxonomy Guide)

> 이 문서는 `_config.yml`의 `exclude:`에 따라 사이트 빌드에서 제외된다. 공개 GitHub 저장소에서는 읽을 수 있으므로 내부 경로·원자료를 넣지 않는다.
> 블로그의 분류 체계(카테고리/태그)와 front matter 규약을 한곳에 정리한다.

---

## 0. 한 줄 원칙

**목적:** 이 블로그는 **주는 사람(giver)의 장**이다 — 경험한 것과 생각하는 것을 담담하게 펴내고 기회가 되면 같은 문제로 막힌 사람을 돕는다. 채용·평가에 주는 신호는 목적이 아니라 **부산물**.

블로그 = **과정(process)** 기둥. 글은 "무엇을 만들었나"가 아니라 **"어떤 대안을 저울질해 왜 그렇게 결정했나"**를 남긴다.
포지셔닝 핵심 = **인공지능 · 로봇 · 신뢰성**. 모든 글은 아래 카테고리 중 하나에 속한다.

**복리 원칙:** 버전이 바뀌어도 남는 **판단·원리·trade-off**를 쓴다. 버전 따라 낡는 글은 도움이 누적되지 않는다 — 오래 남는 글이 독자에게 가장 오래 가는 도움이다. 복리 테스트 — 셋 다 yes여야 발행: ① 2~3년 뒤에도 가치 있나 ② 도구·사건을 떼도 원리가 남나 ③ 내 판단(왜 그렇게 결정했나)이 드러나나.

**가르치는 글:** '가르침' 자체는 금지가 아니다 — 원리가 남는 가르침(왜 이렇게 동작하나, 어디서 막히고 왜인가)은 기버 목적의 정면이다. 쓰지 않는 것 = 버전-의존 클릭 나열형 '셋업/하는 법'(금방 낡아 도움이 안 쌓임). 톤은 위에서 가르치는 훈장 톤 ✗, 같은 문제를 겪은 사람이 같은 눈높이에서 나누는 톤 ○.

**미션 한 줄:** 이 블로그 = 자율 시스템 신뢰성에 대한 *판단*의 **공개·검증 가능한 누적 기록**이자, 그 기록으로 누군가를 돕는 곳. 글 한 편 = 검증 가능한 판단 하나 = 누군가에게 남는 도움 하나.

**독자와의 대화:** About의 초대 문장(막힌 문제 환영)에 응답이 오면 우선해서 답한다 — 댓글(giscus)·LinkedIn. 같은 질문을 두 번 받으면 글감으로 승격한다.

**권위 자산:** 역할은 "핀테크에서 로봇으로 옮길 수 있다"를 *입증하는 증거물*에 그치지 않고 "자율 시스템의 신뢰는 이렇게 설계되어야 한다"는 **관점의 저자**로 인식되는 것이다 — 단 영토는 **신뢰성·컨트롤 플레인에 한정**(자율 시스템 *전체*의 저자 ✗, perception/SLAM/실시간 제어 ✗). 권위는 *닫힌* 단정이 아니라 *열린* 단정에서 나온다: "내가 검증한 범위에서는 이렇게 보며 더 나은 설계가 있으면 듣고 싶다." (※ 경력 단정 경계 = §0 복리 원칙 + 발행 게이트 §4 — "분산 시스템을 다뤄 왔다" 식 과거형 경력 주장 ✗, "설계로 푸는 중"으로.)

RoboRacer의 인지·SLAM·제어 학습과 실험은 직접 수행·검증한 범위에서 기록하며, 이를 해당 분야의 전문성이나 알고리즘 소유 주장으로 확대하지 않는다.

**세 기둥 (글을 배치하는 축):**
- **원칙** — 신뢰성을 읽는 프레임 제시. 권위의 척추. 주로 `신뢰성`. *앵커 글로 깃발을 먼저 꽂되 증명이 빠르게 뒤따라야 한다(원칙이 증명을 앞서면 "말만 하는 사람").*
- **증명** — 그 원칙을 구현한 기록(RoboRacer 프로젝트 등). 앵커를 인용해 실물로 뒷받침. `로봇`·`분산시스템`.
- **현장** — 실전에서 검증한 판단(비식별화한 상태 정합성 경험·도메인 전이). 실전 무게·진정성. `신뢰성`·`분산시스템`.

### 다루는 것 / 다루지 않는 것 (스코프)

**다룬다 (IN):**
- 신뢰성 설계의 판단·trade-off — 상태 정합성·exactly-once·합의·장애 복구·감사/기록 무결성·결정적 재현·관측성. "왜 그렇게 결정했나".
- 도메인을 횡단해 전이되는 원리(결제→로봇→에이전트, **관제 평면 한정**).
- 직접 만든 것의 설계 기록 — RoboRacer·도구 분석 등, 코드/아티팩트 링크와 함께.
- 정직한 한계 — 미구현·신뢰 앵커 한계·버린 것(정직 = 신뢰 신호).
- 새 스택 학습에서 내린 판단·실험 결론("왜·무엇을 배웠나").

**안 다룬다 (OUT):**
- 버전-의존 클릭 나열형 셋업 가이드(금방 낡아 도움이 안 쌓임). *원리가 남는 가르침은 IN — §0을 따른다.*
- 도구 버전 의존 팁(곧 낡음).
- 뉴스·핫테이크·트렌드 코멘터리. *단 트렌딩 도구도 '신뢰성 렌즈 분석'이면 IN.*
- 기밀·고유물(내부 시스템명·고객·incident 상세·정확 수치·보안 갭·원문 코드) — 도메인 뗀 일반 원리만.
- 물리·안전·실시간 제어 알고리즘 '소유' 주장(perception/SLAM/모터 제어). 자리 = fleet 신뢰성·백엔드·컨트롤 플레인. 전이도 "관제 평면 한정".
- 과장·미완성 자랑(진행 중을 완료처럼) — 완료한 것만 단정.
- 개인 일기·감정 토로. *단 작업의 동기·맥락 한두 줄은 진정성 차원에서 IN(절제).*
- 위에서 가르치는 훈장 톤 — 같은 눈높이의 나눔으로(§0 참조).

**애매하면 한 줄 테스트:** "검증 가능한 *판단의 증거*인가, 또는 누군가에게 *남는 도움*인가?" + 복리 테스트 3개. yes면 IN. "기밀"·"곧 낡음"이면 OUT.

사이트 정체성: title = *JAEYEONG's BLOG*, tagline = *Backend Notes: State Mismatch · Duplicate Processing · Partial Failure*. 직함 대신 다루는 실패의 이름으로 쓴다.

---

## 1. 카테고리 (대분류) — 5개 고정

Chirpy에서 카테고리는 별도 등록 파일이 필요 없다. post front matter의 `categories:`만으로 자동 생성된다.
**대분류는 아래 5개로 고정한다. 새 대분류를 늘리지 않는다.**

| 카테고리 | 범위 | 주제 예시 |
| --- | --- | --- |
| **인공지능** | AI·에이전트 쪽. 코딩 에이전트 fleet 신뢰성, 다중 에이전트 오케스트레이션, physical AI 수렴. | 코딩 에이전트 fleet 신뢰성 설계, 에이전트 어댑터, LLM 시스템 운영 |
| **로봇** | 로봇 쪽. RoboRacer 프로젝트 설계·실험 노트, 컨트롤 플레인, VDA5050, AMR 관제. | RoboRacer 개발 기록, 시뮬레이터·실차 실험 회고, VDA5050 어댑터, 관제 인프라 설계 |
| **분산시스템** | 상태 정합성·정확 1회·합의·관측성·장애 대응 등 분산 신뢰성 *주제*. 결제 인프라에서 확인한 패턴(정합성·exactly-once 효과·장애 대응)과 직접 검증한 실험을 다룬다. *주제 카테고리이지 경력 주장이 아니다.* | `state-reconciliation` 멱등성 글, exactly-once 설계, observability 회고 |
| **학습기록** | 새 스택을 익히며 확인한 원리·판단을 설명하는 글. Go·동시성 실험·도구(bench/pprof). | Go 학습 회고, Go race detector 실험, MQTT/조정 루프 학습 글 |
| **신뢰성** | 신뢰성 *자체*를 주제로 다루는 글(도메인 횡단). 감사·결정적 재현·기록 무결성·exactly-once의 일반 원리. | "신뢰의 닻=기록 무결성" 같은 원리 글, 신뢰성 패턴 일반화 |

### 1-1. 카테고리는 한국어

1차 독자가 한국어 사용자(같은 문제로 막힌 실무자 — 동료 엔지니어·채용 담당 포함)이고 사이트 `lang: ko-KR`·본문 한국어와 톤이 맞는다. 영어권 노출은 **제목·태그(영어)·영문 요약·페이지 title**이 담당한다.

### 1-2. Chirpy 카테고리 규약 — 한 글 = 한 대분류

Chirpy에서 `categories: [A, B]`는 **평행 2개가 아니라 "B는 A의 하위"라는 2단계 계층**이다. 5개는 *평행* 대분류이므로:

- **첫 자리에 5개 중 정확히 하나**(그 글의 일차 소속)를 둔다. 둘째 자리는 선택적 하위 카테고리(평행 대분류를 또 넣지 말 것).
- 글이 여러 대분류에 걸치면(흔함) — 일차 하나만 카테고리로 고르고 **나머지는 태그**로 잇는다.
- 특히 **신뢰성**은 거의 모든 글을 관통한다. *신뢰성 원리 자체가 주제*인 글만 `[신뢰성]`에 두고 그 외(로봇/분산시스템 글이 신뢰성을 다룰 때)는 해당 카테고리 + `reliability` 태그.

정본·발행 경로 정합:

| 파일 | categories | 비고 |
| --- | --- | --- |
| `../_posts/2026-06-20-blog-open.md` | `[학습기록]` | 시드/메타 글 |

비공개 원고의 제목·목록·상태는 private `planning/blog-content-roadmap.md`에서만 관리한다. 이 public 저장소는 승인 전 원고를 열거하지 않는다.

---

## 2. 태그 (소분류) 컨벤션

태그 = 5개 카테고리를 가로지르는 평행·교차 축.

- **영어 소문자 + 하이픈**(kebab-case). 예: `exactly-once`, `state-reconciliation`, `control-plane`. permalink(`/tags/:name/`) 안전.
- **글당 3~5개.** 과다 금지.
- 신규 태그는 아래 표에 먼저 등재 후 사용.

| 태그 | 의미 | 주로 붙는 카테고리 |
| --- | --- | --- |
| `reliability` | 신뢰성(횡단 태그 — 신뢰성이 *주제*면 카테고리, *속성*이면 태그) | 인공지능·로봇·분산시스템 |
| `idempotency` | 멱등성 | 분산시스템 |
| `exactly-once` | 정확 1회 전달/처리 | 분산시스템·로봇·신뢰성 |
| `state-reconciliation` | 상태 정합성·대사 | 분산시스템·로봇 |
| `observability` | 관측성·로깅·추적 | 분산시스템·로봇 |
| `consensus` | 합의·리더 선출·fencing/lease | 분산시스템·로봇 |
| `control-plane` | 컨트롤 플레인 설계 | 로봇·인공지능 |
| `audit-log` | 감사 로그·기록 무결성 | 로봇·신뢰성 |
| `deterministic-replay` | 결정적 재현 | 로봇·신뢰성 |
| `fleet` | 로봇/에이전트 fleet | 로봇·인공지능 |
| `vda5050` | VDA5050 프로토콜 | 로봇 |
| `multi-agent` | 다중 에이전트 오케스트레이션 | 인공지능 |
| `golang` | Go 언어 | 학습기록·로봇 |
| `concurrency` | 동시성 | 학습기록·분산시스템 |
| `meta` | 블로그 운영·공지성 | 학습기록 |

> 인물·회사·내부 시스템명은 태그로 쓰지 않는다(기밀·과장 회피).

---

## 3. Front matter 표준 템플릿

```yaml
---
title: 글 제목 (한국어, 담백하게)
date: 2026-06-21 09:00:00 +0900   # KST(+0900). 미래 날짜면 발행 안 됨
categories: [로봇]                 # §1의 5개 중 하나. 하위 필요시 [로봇, 컨트롤플레인]
tags: [control-plane, reliability, fleet] # §2 사전에서, 3~5개, 영어 kebab-case
toc: true
pin: false                         # 대표글에만 true (1개 정도)
# image:                           # 대표 이미지 있을 때만
#   path: /assets/img/.../cover.png
#   alt: 설명
---
```

### 3-1. 본문 구조 (의사결정 서사 기본형)

1. 문제 — 무엇이 어긋나고, 어긋나면 무엇이 깨지나 (구체 장면 훅, 익명화)
2. 왜 어려운가 — 분산/동시성 깊이가 드러나는 지점
3. 첫 시도와 한계
4. 설계 결정 — 고려한 대안 2~3개 → 채택 → 버린 것
5. 결과 — before/after (수치는 일반화: "order-of-magnitude", 정확 비율 ✗)
6. 일반 원리 — 도메인을 떼면 남는 것
7. (선택) 로봇/fleet·에이전트로의 전이 — "전이 가능한 패턴"(동형 단정 ✗)

> 다이어그램이 필요하면 front matter에 `mermaid: true`, 수식은 `math: true` (성능상 글 단위 opt-in, 기본 생략).

### 3-2. 코드·실험 근거 인용 규칙

비공개 블로그 저장소는 원고·편집 계획·원고 검토 기록만 관리한다. 독립 실행 코드와 독서 메모는 보관하지 않는다. 글에 필요한 짧은 코드는 본문 스니펫으로 설명하고 재현 가능한 실험은 공개 승인을 받은 프로젝트의 고정 커밋·실행 조건·결과를 인용한다. 독자가 열 수 없는 비공개 링크를 공개 근거로 쓰지 않는다. 과거 기록만 남았거나 재현 근거가 부족하면 그 한계를 밝히고 검증 완료로 표현하지 않는다.

### 3-3. 영문 요약 (맨 아래 고정)

```markdown
---

## English Summary

> 3~5문장. LinkedIn 공유용. 본문 핵심 결정과 trade-off만.
```

---

### 3-4. 작업 정본과 공개 스냅샷

원고 보관·미리보기·JY 승인·공개 스냅샷 반영은 [저장소 작성 절차](../README.md#writing)를 따른다. 비공개 원고의 승인 상태는 각 원고의 `draft_status`, 목록·다음 편집은 private 콘텐츠 계획에서 관리한다. 아래 발행 게이트는 그 절차에서 검토할 글의 내용·귀속·형식 기준이다.

---

## 4. 발행 게이트 (공개 전 1회 체크)

### 4-0. 온전히 내 것인가 (소유권 게이트)

"온전히 내 것"의 기준은 **완성도(흠 없음·다 끝남)가 아니라 소유권(내가 판단·검증·경계를 안다)**이다. 완성도로 재면 발행을 영원히 막는다(열린 권위는 미완성을 전제하므로 모순). 아래 넷을 통과하면 발행 — "완벽한가"는 게이트에 없다.

- [ ] **판단** — 이 결론을 내가 직접 내렸는가(남의 결론 옮김 ✗)
- [ ] **검증** — 이 근거를 내가 직접 돌려보고/부딪혀봤는가
- [ ] **경계** — 어디까지 검증됐고 어디부터 모르는지 말할 수 있는가
- [ ] **언어** — 빌린 개념이라도 내 문제에 맞춰 다시 짰는가

> 소유권의 *형태*는 글 종류마다 다르다 — **증명 글**은 구현 주체·저장소 검증·JY의 자력 재현 범위를 분리하고 **원칙 글**은 도출 + 귀속이 확인된 구현·현장 경험으로 소유를 증명한다. AI 구현 결과를 JY의 단독 구현으로 바꾸지 않으며 한 기준(특히 "직접 구현한 것만")을 원칙 글에 들이대지도 않는다.
> **온전함의 단위를 작게**: "신뢰성 전체"가 아니라 "이 trade-off 하나"로 좁히면, 신중함을 지키면서 같은 깊이를 빨리 내 것으로 만든다.

### 4-1. 형식·기밀 체크

- [ ] 카테고리 = §1의 5개 중 하나(첫 자리 하나)
- [ ] 태그 = §2 사전 등재 + 3~5개 + 영어 kebab-case
- [ ] 기밀: 내부 시스템명·고객·계약·incident ID·금액·정확 비율·내부 경로 제거(일반화). 원자료의 반출 기준을 확인하고 공개 가능한 설명만 남긴다.
- [ ] 과장 ✗: 완료한 것만 단정. 진행 중인 것은 "설계 방향"·"진행 중" 표기. 저장소 테스트 완료와 JY의 학습·저작 완료를 분리하고 AI 구현 경계를 밝힌다
- [ ] 외부 링크가 열리고 독자가 접근할 수 있는가. 비공개 저장소를 공개 근거로 연결하지 않는다.
- [ ] 영문 요약 포함
- [ ] `date`가 과거/현재 (미래면 미발행)

---

## 5. 사이드바·탭 메모

- Categories·Tags 탭은 실제 발행 글이 사용하는 분류·태그를 표시한다. §1의 허용 카테고리 5개와 §2의 태그 사전을 등록했다고 빈 항목까지 노출되지는 않는다.
- `_tabs/` order: Projects1·About3·Categories4·Tags5·Archives6. 공개 이력서는 GitHub 프로필 저장소의 `RESUME.md`에 둔다.

---

## 6. 미리보기와 검증 안내

실행 명령과 의존성·테마 변경 시 주의점은 [미리보기·검증](../README.md#verification), 배포·외부 링크 검사 조건은 [배포·운영](../README.md#automation)을 따른다.
